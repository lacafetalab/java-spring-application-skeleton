...
import pe.lacafetalab.pao.shared.domain.AuthProvider;
...
public class JsonWebTokenEncriptService implements EncriptService {
	...
	public UserIdentification decript(String token) {
		try {
			...
			UserIdentification response = UserIdentification.builder().userId(new UserId(decoded.getSubject())).role(new Role(decoded.getClaim("role").asString()))
					.provider(new AuthProvider(decoded.getClaim("provider").asString())).build();
